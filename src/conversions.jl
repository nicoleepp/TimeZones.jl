...
"""
    today_at(tod::Time, tz::TimeZone, [amb]) -> ZonedDateTime
...
julia> today_at(Time(10, 30), tz"Europe/Warsaw")
2017-11-09T10:30:00+01:00
```
"""
function today_at(tod::Time, tz::VariableTimeZone, amb::Union{Integer,Bool})
    ZonedDateTime((@mock today(tz)) + tod, tz, amb)
end

today_at(tod::Time, tz::TimeZone) = ZonedDateTime((@mock today(tz)) + tod, tz)
